#!/usr/bin/env python
import rospy
from gymnasium_env.msg import StateReward, Action
import gym_example
import gymnasium
...
class PandaEnvROSNode:
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            state_msg = StateReward(state=state, info=info, reward=0, terminal=False)
            ...
            while not done:
                if self.action is not None:
                    ...
                    next_state, reward, done, _, info = self.env.step(self.action)

                    # Publish current state
                    state_msg = StateReward(state=next_state, info=info, reward=0, terminal=done)
                    self.state_pub.publish(state_msg)
